        ...
        const compact = processedData.map(row => {
            ...
            const active = activeKeys
                .map((k, idx) => ({
                    key: k,
                    val: Number(row[k] || 0),
                    color: pivot === 'machine' ? MONTH_COLORS[idx % MONTH_COLORS.length] : MACHINE_PALETTE[idx % MACHINE_PALETTE.length]
                }))
                            ...
                            <BarChart data={isStacked || showTotalOnly ? processedData : compactData} margin={{ bottom: 60 }} barGap={2}>
                                ...
                                <Tooltip
                                    ...
                                    formatter={(value, name, props) => {
                                        ...
                                        return [value, pivot === 'machine' ? formatMonth(realName) : realName];
                                    }}
                                />
                                <Legend
                                    verticalAlign="top"
                                        ...
                                        <div style={{ display: 'flex', justifyContent: 'flex-end', gap: '15px', flexWrap: 'wrap', marginBottom: '10px' }}>
                                            {activeKeys.map((k, i) => (
                                                <div key={k} style={{ display: 'flex', alignItems: 'center', gap: '6px', fontSize: '12px', fontWeight: '600', color: '#4a5568' }}>
                                                    <div style={{ width: '10px', height: '10px', borderRadius: '3px', backgroundColor: pivot === 'machine' ? MONTH_COLORS[i % MONTH_COLORS.length] : MACHINE_PALETTE[i % MACHINE_PALETTE.length] }} />
                                                    {pivot === 'machine' ? formatMonth(k) : k}
                                                </div>
                                            ))}
                                        </div>
                                    )}
...
export default function ChartOutillage({ from }) {
    ...
    const dashboardData = useMemo(() => {
        ...
        const withTotal = machineKeys.map(machine => {
            const row = { machine };
            let total = 0;
    ...
    const { compactData, maxBars, lastKeysPerX } = useMemo(() => {
        ...
        const compact = dashboardData.map(row => {
            ...
            active.forEach((item, i) => {
                ...
                newRow[`color${i}`] = item.color;
                newRow[`name${i}`] = item.key;
                if (i === active.length - 1) lastKeys[row.machine] = `val${i}`;
            });
            return newRow;
        });
        return { compactData: compact, maxBars: max, lastKeysPerX: lastKeys };
    ...
    return (
        <>
            <ChartContainer title={`Outillage / Machine utilisée (${metric.label})`} onExpand={() => setOpen(true)}>
                {(isExporting) => (
