...
function Frequentations() {
    ...
    const COLUMNS = [
        ...
        { key: "outillage_machine", label: "Outillage" },
    ...
    useEffect(() => {
        ...
        const fetchData = async () => {
            try {
                const [frequentationsRes, projectsRes] = await Promise.all([
    ...
    useEffect(() => {
        if (formData.activite_nom && formData.activite_nom !== 'autre') {
            const selectedProject = projects.find(p => p.intitule_projet === formData.activite_nom);
            if (selectedProject) {
                setFormData(prev => ({
                    ...prev,
    ...
    const isProjectSelected = formData.activite_nom && formData.activite_nom !== 'autre';
    const selectedProject = projects.find(p => p.intitule_projet === formData.activite_nom);
    ...
    const FREQUENTATION_FIELDS = [
        { key: "date", label: "Date", required: false, type: 'date' },
        {
            ...
            options: ['Formation', 'Développement de projet', 'Projet de classe']
        },
        {
            key: "activite_nom", label: "Projet / Activité", required: false, type: 'select',
            options: [
                { label: '--- Autre Activité ---', value: 'autre' },
                ...projects.map(p => ({ label: p.intitule_projet, value: p.intitule_projet }))
            ]
        },
        { key: "activite_pole", label: "Pôle", required: false, disabled: isProjectSelected },
        ...
        { key: "zone_occupee", label: "Zone occupée", required: false },
        { key: "outillage_machine", label: "Outillage", required: false },
    ...
    const handleAddSubmit = async (data) => {
        // data comes from the form state
        const payload = {
            ...
            date: data.date,
            activite: {
                nom: data.activite_nom === 'autre' ? (data.custom_activite_nom || 'Autre Activité') : data.activite_nom,
                pole: data.activite_pole,
                filiere: data.activite_filiere,
                groupe: data.activite_groupe,
            },
            occupation: {
        ...
        // Fallback for custom activity name if 'autre' was selected
        if (data.activite_nom === 'autre' && !data.custom_activite_nom) {
            // We might want to allow the user to type the name too
            // For now, let's add a conditional field in the form if 'autre' is selected
        }
        ...
        try {
            ...
            console.error("Erreur lors de l'ajout de la fréquentation:", err);
        }
    };
